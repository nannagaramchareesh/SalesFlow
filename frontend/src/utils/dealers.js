const RAW_DEALERS = [
  ...
  // Arvind - WED
  { name: 'Eshwar & Electronics Homa Appliances-Alw', salesTeam: 'Arvind', belt: 'WED' }, // wait, text: "Eshwar Electrical & Electronics Homa Appliances-Alw"
...
const uniqueMap = {};
const dedupedDealers = [];

RAW_DEALERS.forEach(d => {
  const key = `${d.name.trim().toLowerCase()}||${d.salesTeam.trim().toLowerCase()}||${d.belt.trim().toLowerCase()}`;
  if (!uniqueMap[key]) {
    uniqueMap[key] = true;
    dedupedDealers.push({
      name: d.name.trim(),
      salesTeam: d.salesTeam.trim(),
      belt: d.belt.trim()
    });
  }
});

export const DEALERS_LIST = dedupedDealers.map(d => ({
  ...d,
  displayName: `${d.name} (${d.belt} - ${d.salesTeam})`
}));

export const getDealerDetails = (name) => {
  if (!name) return null;
  const target = name.trim().toLowerCase();
  return DEALERS_LIST.find(d => d.name.toLowerCase() === target) || null;
};
